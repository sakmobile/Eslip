              ...
              foreach ($getdetail as $row) { ?>
                <tr>
                  ...
                  <td align="center">
                    ...
                    <button class="btn btn-info btn-xs te_info" data-id='<?php echo $row['e_id']; ?>'>
                      <i class="fas fa-eye">
                      </i>
                    ...
                    <button class="btn btn-warning btn-xs edit_te" data-id='<?php echo $row['e_id']; ?>' >
                      <i class="fas fa-pencil-alt">
                      </i>
                    </button>

                    <button class="btn btn-danger btn-xs del_te" data-id='<?php echo $row['e_id']; ?>' >
...
<div class="modal fade" id="edit_te_id">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <form id="form_te" action="" title="" method="post" >
            <div class="modal-body edit_te_show p-3">
             
            </div>
            <div class='modal-footer justify-content-between'>
            <button type='button' class='btn btn-default' data-dismiss='modal'>ยกเลิก</button>
            <button type='submit' id="btn_update" class='btn btn-primary update_te'> <i class='fas fa-save'></i> บันทึก </button>
            </div>
            </form>
          </div>
          <!-- /.modal-content -->
...
<div class="modal fade" id="model_admin_te">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
   ...
   $.ajax({
    ...
    success: function(response){ 
     ...
     $('.body-data').html(response);
      ...
      // Display Modal
      $('#model_admin_te').modal('show'); 
     

    }
  });
});
  $('.edit_te').click(function(){
    var userid = $(this).data('id');
    console.log(userid);
    //$('#edit_offic_id').modal('show');
    // AJAX request
    $.ajax({
     url: '../Admin/Temporary/edit_te',
     type: 'post',
     data: {userid: userid},
     success: function(response){ 
      $('.edit_te_show').html(response);
       // Display Modal
       $('#edit_te_id').modal('show'); 
     }
   });

 });
 $('#form_te').on('submit',function(event){
   event.preventDefault();
   var data = new FormData(this);
   $.ajax({
    url: '../Admin/Temporary/update_te',
    type: 'post',
